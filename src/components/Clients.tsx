import React from 'react';
import { useTranslation } from 'react-i18next';

const ClientsSection: React.FC = () => {
  const { t } = useTranslation();
  const clients = [
    { id: 1, name: 'شاي العروسة', image: '/images/Clients/1.webp' },
    { id: 2, name: 'مكرونة حواء', image: '/images/Clients/2.webp' },
    { id: 3, name: 'مستشفى علاء عزت', image: '/images/Clients/3.webp' },
    { id: 4, name: 'ITCAN', image: '/images/Clients/4.webp' },
    { id: 5, name: 'الشمعدان', image: '/images/Clients/5.webp' },
    { id: 6, name: 'المدرسة المصرية الدولية بـ التجمع الخامس', image: '/images/Clients/6.webp' },
    { id: 7, name: '6 أكتوبر', image: '/images/Clients/7.webp' },
    { id: 8, name: 'NPH', image: '/images/Clients/8.webp' },

  ];

  return (
    <div className="w-full bg-white py-16 px-4 sm:px-6 lg:px-8" dir="rtl">
      <div className="max-w-7xl mx-auto">
        {/* العنوان */}
        <div className="text-center mb-12">
          <h1 className='title text-primary text-2xl'>{t('clientsTitle')}</h1>
        </div>

        {/* شبكة الشعارات */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-6 md:gap-8 items-center">
          {clients.map((client) => (
            <div
              key={client.id}
              className="flex items-center justify-center p-4 bg-white rounded-lg hover:shadow-lg active:shadow-lg transition-all duration-300 transform hover:-translate-y-1 active:-translate-y-1 cursor-pointer group"
            >
              <img
                src={client.image}
                alt={client.name}
                loading='lazy'
                className="w-full h-auto max-h-30 object-contain filter md:grayscale md:group-hover:grayscale-0 transition-all duration-300 md:opacity-70 md:group-hover:opacity-100"
              />
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default ClientsSection;